...
function buildNFA(node) {
  ...
  return { type: 'NFA', alphabet: ['a','b'], states, transitions, meta: {} };
}

function fill(node, from, to, states, transitions) {
  const addState = (id) => {
    states.push({ id, label: id, isStart: false, isAccept: false, position: { x:0, y:0 } });
  };
  const addTrans = (f, t, sym) => transitions.push({ from: f, to: t, symbol: sym });
  ...
  if (node.type === 'LITERAL') {
    ...
    const s1 = newId(), a1 = newId(), s2 = newId(), a2 = newId();
    addState(s1); addState(a1); addState(s2); addState(a2);
